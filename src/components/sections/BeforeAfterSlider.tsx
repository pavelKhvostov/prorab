  ...
  if (!hasBoth) {
    ...
    return (
      <figure className="glass overflow-hidden">
        ...
        {data.label && <figcaption className="border-t border-white/10 px-4 py-3 text-sm text-text-muted">{data.label}</figcaption>}
      </figure>
    );
  }
    ...
    <figure className="glass overflow-hidden">
      ...
      >
        ...
        <div className="absolute inset-0" style={{ clipPath: `inset(0 ${100 - pos}% 0 0)` }}>
          ...
          <span className="pointer-events-none absolute left-3 top-3 rounded-token bg-ink/80 px-2.5 py-1 text-xs font-semibold text-white backdrop-blur-md">
            До
          </span>
        </div>
        ...
        >
          ...
          <span aria-hidden className="relative flex h-10 w-10 items-center justify-center rounded-full bg-white text-ink shadow-token-lg ring-1 ring-black/10">
